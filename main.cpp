...
void memoryProcessesWindow(const char *id, ImVec2 size, ImVec2 position)
{
    ...
    if (ImGui::BeginTabBar("MemoryProcessesTabs"))
    {
        if (ImGui::BeginTabItem("Processes"))
        {
            ...
            if (ImGui::BeginTable("ProcessesTable", 5, flags, ImVec2(0, ImGui::GetTextLineHeightWithSpacing() * 15)))
            {
                ...
                for (const auto &p : processes)
                {
                    ...
                    ImGui::TableNextColumn();
                    // Simplified CPU usage, needs proper calculation over time
                    ImGui::Text("%.2f", 0.0f); // Placeholder for CPU usage
                    ImGui::TableNextColumn();
